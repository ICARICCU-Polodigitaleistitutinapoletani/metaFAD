<?php
class archivi_views_components_DataGridFilter extends pinax_components_Component
{
    function init()
    {
    ...
    public function render($outputMode=NULL, $skipChilds=false)
    {
        ...
        if ($this->getAttribute('parent')) {
            ...
            $arArchiveType = pinax_ObjectFactory::createModel('archivi.models.ArchiveType');
            $arArchiveType->find(array('archive_type_key' => $ar->livelloDiDescrizione));
            $archiveTypeOrder = $arArchiveType->archive_type_order;
        }

        $it = pinax_ObjectFactory::createModelIterator('archivi.models.ArchiveType')
