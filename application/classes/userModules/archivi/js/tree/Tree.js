function treeGen() {
    ...
    $.ajax({
        async: false,
        url: Pinax.ajaxUrl + '&controllerName=archivi.controllers.ajax.GetArchiveType',
        type: 'get',
        success: function (data, textStatus, jqXHR) {
            levelArchive = data.result;
        }
    })
    ...
    $(".gerarchia").fancytree({
        ...
        dnd: {
            ...
            dragEnter: function (node, data) {
                ...
                var targetNode = data.node;
                var sourceType = sourceNode.data.type;
                var targetType = targetNode.data.type;
                if (node.parent !== sourceNode.parent) {
                    if (targetType != 'documento-allegato' && levelArchive[sourceType] >= levelArchive[targetType]) {
                        return ['over'];
                    }
                    return false;
                } else {
            ...
            dragDrop: function (node, data) {
                ...
                $('.ok').click(function () {
                    node.setExpanded(true).always(function () {
                        ...
                        var sourceNodeId = sourceNode.data.id;
                        var targetNodeId = targetNode.data.id;

                        $.ajax({
                            url: Pinax.ajaxUrl + '&controllerName=archivi.controllers.ajax.ModifyTree',
                            type: 'get',
                            action: 'modify',
                            dataType: 'json',
                            data: {
        ...
        source: $.ajax({
            url: Pinax.ajaxUrl + '&controllerName=archivi.controllers.ajax.GetTree',
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
        ...
        lazyLoad: function (event, data) {
            ...
            data.result =
                $.ajax({
                    url: Pinax.ajaxUrl + '&controllerName=archivi.controllers.ajax.GetTreeFromParent',
                    type: 'get',
                    dataType: 'json',
                    data: {
                        id: node.data.id
        ...
        renderNode: function (event, data) {
            ...
            $(node.span).find("#containerImport").remove();
            $(node.span).find("#containerExport").remove();


            var buttonsContainer = $('<div class="btn-container"></div>')
            ...
            var importButton = $('<span id="containerImport"><i class="btn btn-warning btn-flat fa fa-upload"></i></span>');
            var exportButton = $('<span id="containerExport"><i class="btn btn-primary btn-flat fa fa-download"></i></span>');

            buttonsContainer.insertAfter(titolo);

            if (node.data.canAdd) {
                addButton.appendTo(buttonsContainer);
            }
            ...
            editDraftButton.appendTo(buttonsContainer);
            ...
            importButton.appendTo(buttonsContainer);
            if (!node.data.canImport) {
                importButton.find('i').addClass('hidden');
            }
            
            exportButton.appendTo(buttonsContainer);
            if (!node.data.canExport) {
               exportButton.find('i').addClass('hidden');
            }
            ...
            addButton.click(function () {
                ...
                if (id) {
                    ...
                    var parentId = data.node.data.id;
                    $.ajax({
                        url: Pinax.ajaxUrl + '&controllerName=archivi.controllers.ajax.GetLevelType',
                        type: 'get',
                        dataType: 'json',
                        data: {
                            id: parentId,
                    ...
                    $('.ok').click(function () {
                        ...
                        $.ajax({
                            url: Pinax.ajaxUrl + '&controllerName=archivi.controllers.ajax.CreateNodeFromParent',
                            type: 'get',
                            dataType: 'json',
                            data: {
                                parentId: parentId,
            ...
            deleteButton.click(function () {
                ...
                $('.ok').click(function () {
                    ...
                    $.ajax({
                        url: Pinax.ajaxUrl + '&controllerName=archivi.controllers.ajax.Delete',
                        type: 'get',
                        action: 'delete',
                        dataType: 'json',
                        data: {
            ...
            if (node.data.canImport) {
                importButton.click(function () {
                    Pinax.openIFrameDialog(
                        '',
                        'index.php?pageId=archivi-Import_popup&recordId=' + node.data.id,
                        w,
                        $(window).width() / 4,
                        $(window).height() / 10,
            ...
            if (node.data.canExport) {
                exportButton.click(function () {
                    Pinax.openIFrameDialog(
                        '',
                        'index.php?pageId=archivi-Export_popup&recordId=' + node.data.id,
                        w,
                        $(window).width() / 4,
                        $(window).height() / 10,
                        this.openDialogCallback
