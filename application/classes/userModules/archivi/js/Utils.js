function meta_translateToSigla(mod) {
    ...
    else if (mod == pref + "ProgettoDiDigitalizzazione") return "PD";
    return "XX";
}
...
$(document).ready(function () {
    ...
    function normalizeDate(elem, mdu, dateObj) {
        ...
        //Prendo il valore di specifica
        var specifica = parent.find("div.js-archive-cronologia-specifica a span");
        var specificaVal = specifica.text();
        ...
        dateEncoding.prop('value', mdu.encodeDate(dateObj) || mdu.encodeCentury(centuryVal, specificaVal));
    ...
    var isDisabled = $('select[name="tipoData_orig').attr('disabled');
    if ($('#readOnly').val() == "true") {
        isDisabled = 'disabled';
    }
    if (isDisabled !== 'disabled') {
        setInterval(function () {
            $('.js-archive-cronologia-dateType').each(function () {
                var type = $(this).val();
                var estremo = $(this).closest('.form-group').siblings('.js-archive-cronologia-estremo');
                switch (type) {
                    case 'data-singola':
                        setSameValue(estremo, '.js-archive-cronologia-dateInput');
                        updateText(estremo, '.js-archive-cronologia-century', 'data-singola');
                        updateText(estremo, '.js-archive-cronologia-specifica', 'data-singola');
                        updateText(estremo, '.js-archive-cronologia-valid-data', 'data-singola');
                        setSameValue(estremo, '.js-archive-cronologia-note-data');
                        break;
                    case 'data-non-rilevabile':
                        clearValue(estremo[0], '.js-archive-cronologia-dateInput');
                        updateText(estremo, '.js-archive-cronologia-century', 'data-non-rilevabile');
                        updateText(estremo, '.js-archive-cronologia-specifica', 'data-non-rilevabile');
                        updateText(estremo, '.js-archive-cronologia-valid-data', 'data-non-rilevabile');
                        clearValue(estremo[0], '.js-archive-cronologia-note-data');
                        clearValue(estremo[1], '.js-archive-cronologia-dateInput');
                        updateText(estremo, '.js-archive-cronologia-century', 'data-non-rilevabile');
                        updateText(estremo, '.js-archive-cronologia-specifica', 'data-non-rilevabile');
                        updateText(estremo, '.js-archive-cronologia-valid-data', 'data-non-rilevabile');
                        clearValue(estremo[1], '.js-archive-cronologia-note-data');
                        break;
                    case 'data-aperta':
                        clearValue(estremo[1], '.js-archive-cronologia-dateInput');
                        enableField(estremo[0], '.js-archive-cronologia-dateInput');
                        updateText(estremo, '.js-archive-cronologia-century', 'data-aperta');
                        updateText(estremo, '.js-archive-cronologia-specifica', 'data-aperta');
                        updateText(estremo, '.js-archive-cronologia-valid-data', 'data-aperta');
                        clearValue(estremo[1], '.js-archive-cronologia-note-data');
                        enableField(estremo[0], '.js-archive-cronologia-note-data');
                        break;
                    default:
                        enableField(estremo[0], '.js-archive-cronologia-dateInput');
                        enableField(estremo[1], '.js-archive-cronologia-dateInput');
                        enableSelectField($(estremo[0]), '.js-archive-cronologia-century');
                        enableSelectField($(estremo[1]), '.js-archive-cronologia-century');
                        enableSelectField($(estremo[0]), '.js-archive-cronologia-specifica');
                        enableSelectField($(estremo[1]), '.js-archive-cronologia-specifica');
                        enableSelectField($(estremo[0]), '.js-archive-cronologia-valid-data');
                        enableSelectField($(estremo[1]), '.js-archive-cronologia-valid-data');
                        enableField(estremo[0], '.js-archive-cronologia-note-data');
                        enableField(estremo[1], '.js-archive-cronologia-note-data');
                }
            });
        }, 300);
    }
    function setSameValue(estremo, classValue) {
        var inputRemoto = $(estremo[0]).find(classValue);
        var input = inputRemoto.val();
        if (inputRemoto.prop('readonly')) {
            inputRemoto.prop('readonly', false)
        }
        var inputRecente = $(estremo[1]).find(classValue);
        inputRecente.prop('readonly', true);
        ($(estremo[1]).find(classValue)).val(input);
    }

    function updateText(estremo, classValue, parameter) {
        var element = ($(estremo[0]).find(classValue));
        var text = element.find('a span').text();
        if (parameter == 'data-singola') {
            enableSelectField($(estremo[0]), classValue);
            setSelect2Value($(estremo[1]), classValue, text);
        } else if (parameter == 'data-aperta') {
            enableSelectField($(estremo[0]), classValue);
            clearSelectValues($(estremo[1]), classValue);
        } else if (parameter == 'data-non-rilevabile') {
            clearSelectValues($(estremo[1]), classValue);
            clearSelectValues($(estremo[0]), classValue);
        }
    }

    function setSelect2Value(element, classValue, text) {
        var el = $(element).find(classValue);
        var instance = $(el[1]).data('instance');
        instance.$element.select2('disable');
        if (text != instance.getValue()) {
            instance.setValue(text);
            if (text == '') {
                instance.$element.select2('data', text);
                if (classValue == '.js-archive-cronologia-century') {
                    var specifica = $('input[name="estremoRecente_specifica');
                    specifica.closest('div.form-group').hide();
                }
            }
        }
    }

    function clearValue(element, classValue) {
        var field = $(element).find(classValue);
        if (field.val() !== '') {
            field.val('');
        }
    ...
    function clearSelectValues(element, classValues) {
        el = $(element).find(classValues);
        var instance = $(el[1]).data('instance');
        instance.$element.select2('disable');
        if (instance.getValue !== '') {
            instance.$element.select2('data', '');
        }
    }

    function enableSelectField(element, classValue) {
        var el = $(element).find(classValue);
        var instance = $(el[1]).data('instance');
        instance.$element.select2('enable');
    }

    function enableField(element, classValue) {
        var inputRemoto = $(element).find(classValue);
        if (inputRemoto.prop('readonly')) {
            inputRemoto.prop('readonly', false)
        }
    }
    ...
    /**
     * FIX BUG AGGIORNAMENTO VALORE IN LIST READONLY ALL'INTERNO DI UN REPEATER
     */

    var listElement = $('.update-orig-select');
    listElement.each(function () {
        var listElementName = listElement.attr('name');
        var hiddenId = listElementName.replace('_orig', '');
        var updatedVal = $('input[name="' + hiddenId + '"]').val();
        listElement.val(updatedVal);
    });
    ...
    $('[class*="js-pinaxcms-save"]').on('click', setIdentificativo);
