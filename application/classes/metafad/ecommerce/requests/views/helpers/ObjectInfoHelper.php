<?php
class metafad_ecommerce_requests_views_helpers_ObjectInfoHelper extends PinaxObject
{
  public function getInfoFromMetaindex($id,$getLink=true,$isOrderItem=false,$orderItemData=null)
  {
	...
	if($medias)
	{
		$instituteProxy = pinax_ObjectFactory::createObject('metafad_usersAndPermissions_institutes_models_proxy_InstitutesProxy');
		$media = $medias[0];
		$requestDam = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', __Config::get('metafad.dam.solr.url').'select', 'GET', 'q=id:'.$media->id.'&wt=json', 'application/x-www-form-urlencoded');
